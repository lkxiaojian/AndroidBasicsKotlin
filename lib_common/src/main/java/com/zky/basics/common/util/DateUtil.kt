...
object DateUtil {
    ...
    fun getCurrentTime(type: FormatType?): String {
        val sdf = getSimpleDateFormat(type)
    ...
    fun whatDay(date: Date?): String {
        ...
        calendar[year, month] = day // 设置当前时间,月份是从0月开始计算
        val number = calendar[Calendar.DAY_OF_WEEK] // 周表示1-7，是从周日开始，
    ...
    fun formatSecondToHourMinute(duration: Int): String {
        return if (duration < 60) {
            ...
            Math.round(duration.toFloat() / 60).toString() + "分钟"
        } else {
    ...
    fun formatTimeToDay(dateStr: String): String {
        ...
        val sdf2: DateFormat = SimpleDateFormat("MM-dd")
        val sdf3: DateFormat = SimpleDateFormat("yyyy-MM-dd")
        try {
    ...
    fun getLaterTimeByHour(hour: Int): String {
        ...
        now.time = d
        now[Calendar.HOUR] = now[Calendar.HOUR] + hour
    ...
    fun getLaterTimeByDay(date: String?, day: Int): String {
        ...
        calendar[Calendar.HOUR] = calendar[Calendar.HOUR] + day * 24
        val sdf = SimpleDateFormat("yyyy-MM-dd")
    ...
    val currTimePosition: Int
        get() {
            val calendar = Calendar.getInstance()
            calendar.time = Date()
            return Math.ceil((calendar[Calendar.HOUR_OF_DAY] * 60 + calendar[Calendar.MINUTE]).toDouble() / 30)
                .toInt()
        }
